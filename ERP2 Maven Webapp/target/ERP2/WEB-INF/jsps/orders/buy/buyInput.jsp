...
/*
	...
	$(function(){
		$(".supplier").change(function(){
			...
			$.post("orders_ajaxGetGtm.action",{"supplierUuid":supplierUuid},function(data){
				$(".goodsType").empty();
				$(".goods").empty();
		...
		$(".goodsType").live("change",function(){	
			//第一种方式修改对应行的数据：var nowTr = $(this).parent().next().next().children();
			//第二种：
			...
			for(var i = 0; i < goodArr.length; i++){
				used = used + "'" + goodArr[i].value + "',";
			}
			
			$.post("orders_ajaxGetByGtm.action",{"gtmUuid":gtmUuid,"used":used},function(data){
				$gmSelect.empty();
		...
		$(".goods").live("change",function(){
			...
			var gmUuid = $(this).val();
			$.post("orders_ajaxGetGm",{"gmUuid":gmUuid},function(data){
				var price = data.gm.inPriceView;
				//修改数量为1
				$nums.val(1);
				//修改单价
		...
		$("#add").click(function(){
			...
			$.post("orders_ajaxGetGtm2.action",{"supplierUuid":supplierUuid,"used":used},function(data){
			
				$tr = $('<tr bgcolor="#FFFFFF" align="center"></tr>');
				
				$td1 = $('<td height="30"></td>');
				$select = $('<select style="width:110px" class="goodsType"></select>');
				var gtmList = data.gtmList;
				for(var i = 0; i < gtmList.length; i++){
					var gtm = gtmList[i];
					$op = $('<option value="'+gtm.uuid+'">'+gtm.name+'</option>');
				...
				$td2 = $('<td></td>');
				$select = $('<select style="width:110px" class="goods"></select>');
				var gmList = data.gmList;
				for(var i = 0; i < gmList.length; i++){
					var gm = gmList[i];
					$op = $('<option value="'+gm.uuid+'">'+gm.name+'</option>');
				...
				$td3 = $('<td></td>');
				$td3.append('<input name="nums" class="num order_num" style="width:67px;border:1px solid black;text-align:right;padding:2px" value="1" type="text">');
				$tr.append($td3);
				
				$td4 = $('<td></td>');
				var price = data.gm.inPriceView;
				$td4.append('<input name="prices" class="prices order_num" style="width:93px;border:1px solid black;text-align:right;padding:2px" value="'+price+'" type="text"> 元');
				$tr.append($td4);
				...
				$td6 = $('<td></td>');
				$td6.append('<a class="deleteBtn delete xiu" value="4"><img src="images/icon_04.gif"> 删除</a>');
				$tr.append($td6);
				...
				if(gtmList.length == 2 && gmList.length == 2){
					$("#add").css("display","none");
				}
			});
		});
		...
		function total2(){
			...
			for(var i = 0; i < $num.length; i++){
				$sum += $num[i].value * $price[i].value;	
			}
			$(".all").html(intToFloat($sum));
		}
...
<div class="content-right">
	...
	<div class="content-text">
		<s:form action="orders_saveBuy" method="post">
			<div class="square-o-top">
				<table width="100%" border="0" cellpadding="0" cellspacing="0"
					...
					<tr>
						...
						<td width="648px">
							<s:select id="supplier" name="om.sm.uuid" cssClass="supplier" list="supplierList" listKey="uuid" listValue="name"></s:select>
						</td>
						<td height="30">
